...
export default function RegisterData() {
    ...
    const handleRegister = (e: FormEvent<HTMLFormElement>) => {
        ...
        const valiPassword = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/;
        ...
        // Banco de dados.
        const savedUsers: User[] = JSON.parse(localStorage.getItem('db_users') || "[]");
    ...
    return (
        <form onSubmit={handleRegister} className="w-full max-w-md min-h-[450px] shadow-2xl flex flex-col p-10 rounded-xl bg-slate-900">
            ...
            <div className="flex w-full flex-col gap-4">
                ...
                <button type='submit' className="w-full h-12 mt-4 bg-yellow-500 hover:bg-yellow-800 cursor-pointer text-white font-bold rounded-md transition-all shadow-lg active:scale-95">
                    Cadastrar
                </button>

                <button onClick={() => navigate("/login-usuario")}
